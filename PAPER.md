...
1. **3D-CNN encoder-decoder:** a volumetric U-Net-style backbone using Conv3d, InstanceNorm, and LeakyReLU blocks.
2. **Denoising autoencoder pretraining:** the encoder is first trained to reconstruct clean volumes from corrupted inputs.
3. **ConvLSTM temporal block:** bottleneck features can be processed across cine frames when temporal input is provided.
